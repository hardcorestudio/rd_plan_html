'use strict';
angular.module('sbAdminApp').controller('AgreementListCtrl', ['$scope','Init','localStorageService','$state','CheckBrowser','Modal','CheckParam', function ($scope,Init,localStorageService,$state,CheckBrowser,Modal,CheckParam) {
    CheckBrowser.check();
    ...
    //table数据
    $scope.table = $('#agreementTable').DataTable({
        serverSide: true,
        "columns": [
            {
                "data": "AM_ID"
        ...
        ajax:function(data, callback, settings){
            ...
            //查询协议列表
            Init.iwbhttp('/agreement/queryAgreementList', $scope.param, function(data,header,config,status){
                var returnData = {};
                if(data.resFlag == 0){
    ...
    //查看详情
    $('#agreementTable tbody').on('click', '#a_check', function () {
        var row = $scope.table.row($(this).parents('tr'));
        var data = row.data();
        $state.go("dashboard.agreementIndex.agreementInfo", {agreementId:data.AM_ID,from:"dashboard.agreementIndex.agreementList"});
    });
    ...
    $scope.postService = function (){
        ...
        var url = "/agreement/queryAgreementList";
        ...
        Init.iwbhttp(url, $scope.param, function (data, header, config, status) {
            if (data.resFlag == 0) {
                ...
                {
                    $scope.table.row(i).columns = data.dataList[i];
                }
                $scope.table.draw();
            } else {
                $scope.open(data.msg);
    ...
    //新增
    $scope.add = function() {
        $state.go("dashboard.agreementIndex.agreementInfo", {agreementId:"",from:"dashboard.agreementIndex.agreementList"});
    }
    //提示modal弹框
