'use strict';
angular.module('sbAdminApp').controller('AgreementListCzCtrl', ['$scope','Init','localStorageService','$state','CheckBrowser','Modal','CheckParam', function ($scope,Init,localStorageService,$state,CheckBrowser,Modal,CheckParam) {
    CheckBrowser.check();
    ...
    table = $('#agreementTable').DataTable({
        ...
        ajax:function(data, callback, settings){
            ...
            Init.iwbhttp('/agreement/queryAgreementListForCz', $scope.param, function(data,header,config,status){
                var returnData = {};
                if(data.resFlag == 0){
    ...
    $('#agreementTable tbody').on('click', '#a_check', function () {
        ...
        var data = row.data();
        $state.go("dashboard.agreementIndexCz.agreementInfoCz", {agreementId:data.AM_ID,from:"dashboard.agreementIndexCz.agreementListCz"});
    });
    ...
    $scope.postService = function (){
        ...
        var url = "/agreement/queryAgreementListForCz";
