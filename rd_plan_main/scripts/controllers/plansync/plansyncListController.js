'use strict';
angular.module('sbAdminApp').controller('PlansyncListCtrl', ['$scope','$rootScope','$state','Init','CheckBrowser','localStorageService','CheckParam','Modal','$stateParams','$location', function ($scope,$rootScope,$state,Init,CheckBrowser,localStorageService,CheckParam,Modal,$stateParams,$location) {
    ...
    $scope.applyBtn = function(){
        ...
        var sub_url = $rootScope.subSyncUrl+"?IWBSESSION="+localStorageService.get('IWBSESSION')+"&WJWT="+localStorageService.get('WJWT')+"&DEVICE_UUID="+$rootScope.uuid+"&CURRENT_URL="+$location.url()+"&USER_ID="+localStorageService.get('userId')+"&TP_ID="+$stateParams.tpId+"&EP_ID="+$stateParams.epId+"&showflag=true&epName="+$scope.epName+"&SYNC_TYPE=OUT_ADD" ;
    ...
    $scope.approve = function (parm){
        url = 'views/dashboard/plansync/approvalModal.html';
        ctrlName = 'ApprovalModalCtrl';
        resolve = {
            parm: function () {
                return parm;
            },
