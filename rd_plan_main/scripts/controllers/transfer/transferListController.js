'use strict';
angular.module('sbAdminApp').controller('TransferListCtrl', ['$scope','Init','Modal','localStorageService','$state','CheckBrowser','CheckParam', function ($scope,Init,Modal,localStorageService,$state,CheckBrowser,CheckParam) {
    CheckBrowser.check();
    ...
    //table数据
    $scope.table = $('#transferPlanTable').DataTable({
        "serverSide": true,
        "columns": [
            {
                "data": "TP_ID"
        ...
        ajax:function(data, callback, settings){
            ...
            //查询计划列表
            Init.iwbhttp('/plan/queryPlanListByApply', $scope.param, function(data,header,config,status){
                var returnData = {};
                if(data.resFlag == 0){
    ...
    //查看详情
    $('#transferPlanTable tbody').on('click', '#a_check', function () {
        var row = $scope.table.row($(this).parents('tr'));
        var data = row.data();
        $state.go("dashboard.transferIndex.transferInfo", {transferId:data.TP_ID,from:"dashboard.transferIndex.transferList"});
    });
