'use strict';
angular.module('sbAdminApp').controller('TransferListForAdminCtrl', ['$scope','Init','Modal','localStorageService','$state','CheckBrowser','CheckParam', function ($scope,Init,Modal,localStorageService,$state,CheckBrowser,CheckParam) {
    CheckBrowser.check();
    ...
    var table;
    //table数据
    table = $('#transferPlanForAdminTable').DataTable({
        "serverSide": true,
        "columns": [
            {
                "data": "TP_ID"
        ...
        ajax:function(data, callback, settings){
            ...
            //根据管理员信息查询计划列表
            Init.iwbhttp('/plan/queryPlanListForAdmin', $scope.param, function(data,header,config,status){
                var returnData = {};
                if(data.resFlag == 0){
    ...
    //查看详情
    $('#transferPlanForAdminTable tbody').on('click', '#a_check', function () {
        var row = table.row($(this).parents('tr'));
        var data = row.data();
        $state.go("dashboard.transferForAdminIndex.transferForAdminInfo", {transferId:data.TP_ID,from:"dashboard.transferForAdminIndex.transferForAdminList"});
    });
